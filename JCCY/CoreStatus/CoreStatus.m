...
@property (nonatomic,strong) CoreReachability *reachability;
...
-(CoreReachability *)reachability{
    ...
    if(_reachability == nil){
        ...
        _reachability = [CoreReachability reachabilityForInternetConnection];
    }
...
+(void)beginNotiNetwork:(id<CoreStatusProtocol>)listener{
    ...
    [[NSNotificationCenter defaultCenter] addObserver:status selector:@selector(coreNetWorkStatusChanged:) name:kCoreReachabilityChangedNotification object:nil];
...
+(void)endNotiNetwork:(id<CoreStatusProtocol>)listener{
    ...
    //解除监听
    [[NSNotificationCenter defaultCenter] removeObserver:status name:kCoreReachabilityChangedNotification object:nil];
