...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (pickerImageView.isMaskViewFlag) {
        ...
        // 1 判断图片数超过最大数或者小于0
        NSUInteger minCount = (self.minCount < 0) ? KPhotoShowMaxCount :  self.minCount-self.selectedCount;
        NSLog(@"minCount:%ld",minCount);
        NSLog(@"selectedCount:%ld",self.selectedCount);
