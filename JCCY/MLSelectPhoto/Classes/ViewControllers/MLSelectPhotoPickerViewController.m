...
- (void)setSelectedCount:(NSInteger)selectedCount
{
    ...
    self.groupVc.selectedCount = selectedCount;
}
...
+ (UIImage *)getImageWithHDImageObj:(id)imageObj{
    __block UIImage *image = nil;
    if ([imageObj isKindOfClass:[UIImage class]]) {
        return imageObj;
    }else if ([imageObj isKindOfClass:[ALAsset class]]){
        @autoreleasepool {
            ...
            image = [MLSelectPhotoPickerViewController imageCompressForSize:image targetSize:CGSizeMake(image.size.width, image.size.height)];
...
//        ALAssetRepresentation* representation = [asset defaultRepresentation];
        //获取资源图片的高清图
        UIImage *image = [imageObj originImage];
        image = [MLSelectPhotoPickerViewController imageCompressForSize:image targetSize:CGSizeMake(image.size.width, image.size.height)];
        return image;
    }
    return image;
