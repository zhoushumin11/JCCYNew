...
@property (assign) NSInteger textViewMaxLeght;
            ...
            dismissBlock:(dispatch_block_t)dismissBlock
{
    ...
    switch (type) {
        case EYInputPopupView_Type_single_line_text:
        {
            ...
            popView.tfContent.layer.borderColor=COLORRGB(0xaeeeeee).CGColor;
            popView.tfContent.layer.borderWidth=0.5;
            popView.textViewMaxLeght = maxLeght;
            popView.tfContent.keyboardType = keyboardType;
            popView.tfContent.textAlignment = NSTextAlignmentLeft;
            popView.tfContent.textColor = [UIColor colorWithRed:127.0/255.0 green:127.0/255.0 blue:127.0/255.0 alpha:1];
            popView.tfContent.font = [UIFont systemFontOfSize:15.0f];
            popView.tfContent.backgroundColor=[UIColor clearColor];
...
{
    if (textField == self.tfContent) {
        if (string.length == 0) return YES;
        
        NSInteger existedLength = textField.text.length;
        NSInteger selectedLength = range.length;
        NSInteger replaceLength = string.length;
        if (existedLength - selectedLength + replaceLength > textViewMaxLeght) {
            return NO;
        }
    }
...
-(void)textViewDidChange:(UITextView *)textView{
    ...
    if (number > textViewMaxLeght) {
        NSString *maxstr = [NSString stringWithFormat:@"字符个数不能大于%ld",textViewMaxLeght];
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:maxstr delegate:self cancelButtonTitle:@"确定" otherButtonTitles:nil];
        [alert show];
        textView.text = [textView.text substringToIndex:textViewMaxLeght];
        number = textViewMaxLeght;
    }else{
        [self resetFrame];
    }
}
