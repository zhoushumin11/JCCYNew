...
- (void)buildFooterView {
    self.pageControl = [[UIPageControl alloc] initWithFrame:CGRectMake(0, [UIScreen mainScreen].bounds.size.height - self.pageControlY, self.frame.size.width, 20)];
    [self.pageControl setAutoresizingMask:UIViewAutoresizingFlexibleWidth];
    ...
    self.pageControl.pageIndicatorTintColor = [UIColor colorFromHexRGB:@"f0fbff"];
    self.pageControl.currentPageIndicatorTintColor = [UIColor colorFromHexRGB:@"#098ae6"];
//    [self addSubview:self.pageControl];
    
    self.skipButton = [[UIButton alloc] initWithFrame:CGRectMake(0, PPMainViewHeight - 200, PPMainViewWidth, 200)];
...
- (void)showInView:(UIView *)view animateDuration:(CGFloat)duration button:(NSString *)buttonTitle{
    self.alpha = 0;
    if(buttonTitle != nil){
